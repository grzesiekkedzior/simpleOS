...
void cpu_detect() {
    ...
    u32int largestStandardFunc;
    char vendor[13];
    cpuid(0, &largestStandardFunc, (u32int *)(vendor + 0), (u32int *)(vendor + 8), (u32int *)(vendor + 4));
    vendor[12] = '\0';
    ...
    if (largestStandardFunc >= 0x01) {
        ...
        if (edx & EDX_PSE)      monitor_write(" PSE");
        if (edx & EDX_PAE)      monitor_write(" PAE");
        if (edx & EDX_APIC)     monitor_write(" APIC");
        if (edx & EDX_MTRR)     monitor_write(" MTRR");

        monitor_write("\n");

        monitor_write("Instructions:");

        if (edx & EDX_TSC)      monitor_write(" TSC");
        if (edx & EDX_MSR)      monitor_write(" MSR");
        if (edx & EDX_SSE)      monitor_write(" SSE");
        if (edx & EDX_SSE2)     monitor_write(" SSE2");
        if (ecx & ECX_SSE3)     monitor_write(" SSE3");
        if (ecx & ECX_SSSE3)    monitor_write(" SSSE3");
        if (ecx & ECX_SSE41)    monitor_write(" SSE41");
        if (ecx & ECX_SSE42)    monitor_write(" SSE42");
        if (ecx & ECX_AVX)      monitor_write(" AVX");
        if (ecx & ECX_F16C)     monitor_write(" F16C");
        if (ecx & ECX_RDRAND)   monitor_write(" RDRAND");
    ...
    if (largestExtendedFunc >= 0x80000004) {
        ...
        cpuid(0x80000004, (u32int *)(name + 32), (u32int *)(name + 36), (u32int *)(name + 40), (u32int *)(name + 44));
